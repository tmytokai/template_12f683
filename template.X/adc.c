...
void init_adc(void)
{
    ...
    // アナログ入力モードにする場合は対応する TRISIO も入力モードにして下さい
    // GP0〜GP2はAN0〜AN2に対応していますが、GP4 = AN3 なのに注意
    // 詳しくはデータシート 35 pを参照してください
    ANSEL = ANSEL | ? ;

    // ADCのクロック設定です
    // 早すぎると正しく変換できないので、64p 表 9-1 で影が入ってないマス目のスピードを選びます
    ANSELbits.ADCS = 0b001;  // FOSC=4MHzの時は FOSC/8 が最適

    // 1 をセットするとADCモジュールがONになります
    // 詳しくはデータシート 67 p を参照してください
    ADCON0bits.ADON = 1 ;

    // 結果の出力形式を右詰めにするか左詰めにするかの設定です
    // 1が右詰め、0が左詰めです
    // 今回は右詰めにします
    // 詳しくはデータシート 68p を参照してください
    ADCON0bits.ADFM = 1 ;
}
